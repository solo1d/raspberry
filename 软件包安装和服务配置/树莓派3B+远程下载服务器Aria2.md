...
- 首先:
  - 创建文件夹`mkdir /root/.aria2c`  
  - 新建记录文件 `touch /root/.aria2c/aria2.session`
  - 然后 添加一个aria配置文件`vim /root/.aria2c/aria2.conf`
...
#rpc-passwd=passwd
#设置加密的密钥
rpc-secret=aaaaaa12364728126123
#允许rpc
enable-rpc=true
#允许所有来源, web界面跨域权限需要
...
> 注意 ：设置好配置，还要创建该会话空白文件 `touch /root/.aria2c/aria2.session` 

>  测试下aria2是否启动成功 `aria2c --conf-path=/root/.aria2c/aria2/aria2.conf` 
...
`wget https://github.com/mayswind/AriaNg/releases/download/1.3.7/AriaNg-1.3.7.zip -O aira-ng.zip`
...
![image-20240106114245879](./assets/image-20240106114245879.png)
...
脚本项目地址：  https://github.com/P3TERX/aria2.sh

执行这个命令就可以了  `wget -N git.io/aria2.sh && chmod +x aria2.sh`
...
# RPC 密钥
rpc-secret=c53b792fc86e180d6322
